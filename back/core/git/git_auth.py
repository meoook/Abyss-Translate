...
class AuthConnector:
    _abyss_auth = {'access_type': '', 'access_token': ''}

    def get_access_token(self, access_type, access_token=None):
        """ Get token by type - abyss access if type not defined (abyss account must be registered in git provider) """
        _type, _token = access_type, access_token
        if not _token or _type not in ['oauth', 'ssh', 'token']:
            _type, _token = self._abyss_auth['access_type'], self._abyss_auth['access_token']
        if _type == 'oauth':
            return self._oauth_get_access_token_by_refresh(access_token)
        return access_token  # for token and ssh types
    ...
    def _oauth_get_access_token_by_refresh(self, refresh_token):
        """
            ...
            Some providers don't have refresh system. For them: access_token = refresh_token
        """
...
class AuthGitHub(AuthConnector):
    _abyss_auth = {'access_type': 'token', 'access_token': 'a64a7ef131c0a8823e9f4b8228b86f5c077fd6b9'}
...
class AuthBitBucket(AuthConnector):
    _abyss_auth = {'access_type': 'oauth', 'access_token': 'dT3cWh4p3QxvMf4WnG'}
...
class AuthGitLab(AuthConnector):
    _abyss_auth = {'access_type': 'token', 'access_token': 'ZocaYQAp9UYd18wk1Psk'}
