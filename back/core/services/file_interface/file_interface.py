...
from core.services.file_interface.file_closed_api import FileClosedApi
from core.services.file_interface.file_related_model import FileRelatedModelApi


class FileInterface(FileClosedApi):
    ...
    def translate_change_by_user(self, translator_id, trans_id, text, md5sum=None, **_kwargs) -> tuple[dict, int]:
        ...
        if md5sum:  # multi update
            ...
            for translate_obj in Translate.objects.filter(**filter_options):
                model_api.translate_change_by_user(translate_obj, text, user_id=translator_id)
        else:
            model_api.translate_change_by_user(translate, text, user_id=translator_id)
