...
class LocalizeFileManager:
    ...
    def parse(self):
        ...
        file_info = DataGetInfo(self.__data_binary, self.__lang_orig.short_name)
        if file_info.info['error']:
            logger.warning(f"Error getting info for file ID: {self.__work_file.id}")
            self.__error_file_msg = file_info.info['error']
            return False
        # Rebuild file if exist translations or build new
        rebuild = True if self.__work_file.method else False
