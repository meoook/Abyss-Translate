...
class DeleteInOS:
    ...
    def __delete_folder(folder_path: str) -> bool:
        ...
        if os.path.isdir(_root_path):
            ...
            try:
                ...
                logger.info(f'OS delete root folder {_root_path} - success')
                return True
        else:
            logger.error(f'OS not found directory path:{_root_path} to delete')
        return False

    @staticmethod
    def __os_del_file(_path: str) -> bool:
        logger.debug(f'OS called to delete file:{_path}')
    ...
    def __os_del_folder(_path: str) -> bool:
        logger.debug(f'OS called to delete folder:{_path}')
