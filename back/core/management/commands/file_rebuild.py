...
class Command(BaseCommand):
    help = 'Rebuild translates after file update'
    __marks_to_check, __marks_to_update, __marks_to_add = None, None, None

    def add_arguments(self, parser):
        parser.add_argument('id', type=int, default=None)

    def handle(self, *args, **options):
        # Reset state - cos function cash this values
        ...
        if len(self.__marks_to_check) == 0:
            self.stdout.write('Build file marks {}(id:{}) language: {} method: {} options: {}'.format(*file_options))
            rebuild = False
        else:
            rebuild = True
            self.stdout.write('Rebuild file marks {}(id:{}) language: {} method: {} options: {}'.format(*file_options))
