...
const AppState = ({ children }) => {
  ...
  const loading = (loadState) =>
    dispatch({ type: INIT_LOADER, payload: loadState })
  ...
  const prjUpdate = async (project) => {
    try {
      const res = await axios.put(
        `${URL}/prj/${project.save_id}/`,
        project,
        config
      )
      const payload = state.projects.map((prj) =>
        prj.save_id !== res.data.save_id ? prj : res.data
      )
  ...
  const fldrUpdate = async (folder) => {
    try {
      const res = await axios.put(
        `${URL}/prj/folder/${folder.id}/`,
        folder,
        config
      )
      const payload = state.folders.map((fldr) =>
        fldr.id !== res.data.id ? fldr : res.data
      )
  ...
  const fileUpdate = async (fileObj) => {
    try {
      ...
      const payload = state.explorer.results.map((file) =>
        file.id !== res.data.id ? file : res.data
      )
  ...
  const fileRemove = async (file_id) => {
    try {
      ...
      const payload = state.explorer.results.filter(
  ...
  const transChange = async (trans_id, text, md5sum = '') => {
    ...
    try {
      ...
      const payload = state.translates.results.map((transItem) => {
        ...
        const haveTransBefore = transItem.translates_set.find(
          (item) => item.id === trans_id
        )
        if (!haveTransBefore)
          return {
            ...transItem,
  ...
  const permAdd = async (save_id, first_name, permission) => {
    try {
      const res = await axios.post(
        `${URL}/prj/perm/`,
        { save_id, first_name, permission },
        config
      )
      let user = state.permissions.find(
        (item) => item.first_name === first_name
      )
  ...
  const permRemove = async (save_id, first_name, permission) => {
    ...
    try {
      ...
      user.prj_perms = user.prj_perms.filter(
