      ...
      <div className={`scroll-y paginate column m-1${!trOnly ? ' ml-3' : ''}`}>
        {loading ? (
          ...
          <div className='col col-4'>
            {trOnly ? <NoFilesList /> : fldrID ? <NoFilesInfo /> : <></>}
          </div>
        ) : (
          <>
            {fileList.map((item) => (
...
const NoFilesInfo = (perms) => {
  return (
    <div className='card card-info'>
      ...
      <div>
        В "настройках папки" так же можно поменять название папки и указать гит
        репозиторий
      </div>
    </div>
  );
...
const NoFilesList = (perms) => {
  return (
    <div className='card card-info'>
      ...
      <div>
        Владелец игры должен загрузить файлы, после чего они будут отображаться
        в списке
      </div>
    </div>
  );
